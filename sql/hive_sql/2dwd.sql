...
select
    ...
    feight_fee           ,
    final_total_amount-ods_order_info.feight_fee,
    dt
...
-- ① 建表过程自主使用数据存储格式、压缩格式（3分）
-- ② 该表需要按天分区 （3分）
-- ③ 列的分隔使用’\t’（3分）
-- ④  指定存储位置为 ‘/2207A/runbo_zhang/gmall/dwd/表名’（3分）
-- ⑤  编写HQL语句向DWD层用户行为宽表（3分）
-- ⑥  查询主表和相关表，显示数据加载成功（3分）
select *from dwd_order_detail ;
select *from dwd_order_info ;
select *from dwd_user_info ;
select *from dwd_sku_info ;